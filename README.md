...
GET  /wal                                                    →  {records, checkpointLSN, hasCheckpoint}
POST /wal/checkpoint                                         →  {lsn, message}
POST /wal/crash                                              →  {ok, message}  — revert DB to last checkpoint
POST /wal/recover                                            →  {replayed, message}  — replay WAL since checkpoint
```
...
Every DML statement and transaction event appends a `WALRecord` to an in-memory log. Each record carries:
- **LSN** — monotonically increasing log sequence number
- **XID** — transaction ID (0 for auto-commit)
- **Op** — `INSERT`, `UPDATE`, `DELETE`, `BEGIN`, `COMMIT`, `ROLLBACK`, or `CHECKPOINT`
- **Table**, **OldRows**, **NewRows** — data for redo

The three-step crash/recovery demo:

1. **Checkpoint** — snapshots all table pages and tx state into `WALManager.checkpoint`. Writes a `CHECKPOINT` record to the log.
2. **Crash** — wipes in-memory DB back to the checkpoint snapshot. WAL records are preserved (they represent the "durable" log).
3. **Recover** — replays all WAL records written after the checkpoint: re-inserts rows, re-applies updates, re-deletes rows, restores tx commit/abort state.

WAL records are generated by the executor (not storage): `execInsert/execUpdate/execDelete` call `db.WAL.Append` after each successful storage operation. `execBegin/execCommit/execRollback` log their events immediately. The `WALManager` lives at `internal/storage/wal.go`.
